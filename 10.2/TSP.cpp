...
int main(int argc, char * argv[]){
  ...
  //Parallel initialize
  MPI_Init(&argc,&argv);
  MPI_Comm_size(MPI_COMM_WORLD, &size_par);
  MPI_Comm_rank(MPI_COMM_WORLD, &rank_par);
  ...
  for(int istep = 1; istep<=n_steps; istep++)
    {
      ...
      if(istep%100 == 0)
	{
	   cout << "(" << rank_par << ") " << "Number of steps: " << istep
      	       << "/" << n_steps << "  fitness_min = "
	       << population[0][n_genes] << endl;
	 
	}
      if(istep%25 == 24) Migration();
      Measure(istep);
    }
...
void Input(void)
{
  ...
  for(int i = rank_par; i < size_par; i++)
    Primes >> p1 >> p2 ;

  cout << endl << "(" << rank_par << ") Primes: "
       << p1 << " " << p2 << endl;
  ...
  if(rank_par == 0)
    {
      ...
      cout << endl << "Probability of cross-over: "
	   << probability_crossover * 100 << "%" << endl;  

      cout << "Probability of mutation: "
	   << probability_mutation * 100 << "%" << endl;
...
void Measure(int istep)
{
  ...
  Ave.open("output.ave."+to_string(rank_par),ios::app);
  Min.open("output.min."+to_string(rank_par),ios::app);
...
void Write_Best_Path()
{
  ...
  Path.open("output.path."+to_string(rank_par));
...
void Migration(void)
{
  double migrants[size_par][n_genes + 1];

  for(int i = 0; i < size_par; i++)
    {
      for(int j = 0; j <= n_genes; j++)
	migrants[i][j] = population[0][j];

      MPI_Bcast(migrants[i],n_genes+1,MPI_DOUBLE,i,MPI_COMM_WORLD);
    }

  for(int i = 0; i < size_par; i++)
    for(int j = 0; j <= n_genes; j++)
      population[i][j] = migrants[i][j];
