...
//Read input informations
  ...
  ReadInput >> restart; // if=1 restart from config.final
...
//initial configuration
  if(restart == 1){
    cout << endl << "Restart from config.final" << endl;
...
void Results(void) //Writing on the file results the temperature and the
                   //corresponding measures with errors 
{
  ofstream Ene,Mag,Chi,Heat;
  if(h <= 0.01)
    {
      Ene.open("results.ene.csv", ios::app);
      Ene << temp << "," << stima_u << "," << err_u << endl;
      Ene.close();

      Heat.open("results.heat.csv", ios::app);
      Heat << temp << "," << stima_c << "," << err_c << endl;
      Heat.close();

      Chi.open("results.chi.csv", ios::app);
  ...
  if(h > 0.01)
    {
      Mag.open("results.mag.csv", ios::app);
...
double Error(double sum, double sum2, int iblk)
{
    ...
    else return sqrt(fabs((sum2/(double)iblk - pow(sum/(double)iblk,2))/(double)(iblk-1)));
}
